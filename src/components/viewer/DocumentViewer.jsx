...
import {
  ChevronLeft, ChevronRight, Download, FileText, Link, Minus, Plus, X,
  Sparkles, Search, MoreVertical, Maximize2, Minimize2, RefreshCcw
} from 'lucide-react';
...
function highlightPlainText(text, term, matchOffset = 0, currentMatch = -1) {
  ...
  while (index !== -1) {
    ...
    nodes.push(<mark key={`${index}-${needle}-${matchNumber}`} data-search-match={matchNumber} className={`pdf-search-highlight ${matchNumber === currentMatch ? 'pdf-search-highlight-current' : ''}`}>{text.slice(index, index + needle.length)}</mark>);
    matchNumber += 1;
...
function buildDocumentPages(type, text, html, pageCount) {
  ...
  if (type === 'txt') {
    ...
    sourceLines.forEach(line => {
      const lineCount = Math.max(1, Math.ceil(Math.max(1, line.length) / charsPerRenderedLine));
      if (current.length && renderedLineCount + lineCount > renderedLinesPerPage) pushCurrent();
      ...
      renderedLineCount += Math.max(1, Math.ceil(Math.max(1, remaining.length) / charsPerRenderedLine));
    });
  ...
  if (type === 'docx') {
    ...
    return Array.from({ length: count }, (_, i) => blocks.slice(i * perPage, (i + 1) * perPage).map(node => node.outerHTML).join('')).filter((_, i, a) => a[i] || i === 0);
  }
...
export default function DocumentViewer({ doc, jumpPage, onClose }) {
  ...
  const actualPageCount = pdf?.numPages || (isWordDocument(doc) ? wordPageCount : documentPages.length) || 1;
  ...
  useEffect(() => {
    ...
    const updateSelection = () => {
      ...
      selection.style.transform =
        `translate3d(${boxRect.left - contentRect.left}px, ${boxRect.top - contentRect.top}px, 0)`;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const node = pageRefs.current.get(Math.max(1, Math.min(actualPageCount, jumpPage)));
  ...
  useEffect(() => {
    setPage(Math.max(1, Math.min(jumpPage || 1, pdf?.numPages || doc?.pages || 1)));
    ...
    async function loadPdf() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setRenderedPages(new Set([first, first - 1, first + 1].filter(n => n >= 1 && n <= loaded.numPages)));
        }
      } catch {
  ...
  useEffect(() => {
    ...
    root.addEventListener('scroll', handleProgrammaticScroll, { passive: true });
  ...
  useEffect(() => {
    ...
    const syncFromDocumentScroll = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const rail = thumb.closest('[data-page-scroller]') || thumb.parentElement;
        if (!rail) return;
        ...
        if (Math.abs(thumbCenter - railCenter) > Math.max(4, railRect.height * 0.08)) {
          const desired =
  ...
  useEffect(() => {
    ...
    if (doc.type === 'txt') {
      fetch(doc.url)
        .then(r => r.ok ? r.text() : Promise.reject(new Error('Unable to load text document')))
        .then(text => {
      ...
      fetch(doc.url)
        .then(r => r.ok ? r.arrayBuffer() : Promise.reject(new Error('Unable to load Word document')))
        .then(arrayBuffer => {
          if (cancelled) return;
  ...
  useEffect(() => {
    ...
    source.innerHTML = highlightHtml(viewerHtml, viewerSearch, wordMatchOffset, searchIndex);
    ...
    const collect = node => {
      Array.from(node.childNodes).forEach(child => {
        ...
        if (/^(P|H1|H2|H3|H4|H5|H6|TABLE|UL|OL|IMG|BLOCKQUOTE|PRE|HR)$/.test(tag)) {
    ...
    const createPage = () => {
      ...
      page.className = 'word-a4-page pdf-paper relative shrink-0 overflow-hidden shadow-sm';
  ...
  }, [viewerHtml, viewerSearch, viewerSearchActive, searchIndex, doc?.id, doc?.type, doc?.name]);
  ...
  useEffect(() => {
    ...
    if (pdf) {
      (async () => {
        ...
        if (!cancelled) {
          ...
          if (matches.length) {
            const targetPage = matches[Math.min(searchIndex, matches.length - 1)]?.page;
            if (targetPage) {
              setRenderedPages(prev => {
                const next = new Set(prev);
      ...
      const root = wordViewerRef.current;
      const marks = root ? Array.from(root.querySelectorAll('[data-search-match]')) : [];
      ...
      setSearchIndex(prev => Math.min(prev, Math.max(0, pageMatches.length - 1)));
    } else {
      const matches = [];
    ...
    return () => { cancelled = true; };
  }, [pdf, doc?.id, doc?.name, viewerText, viewerHtml, viewerSearch, viewerSearchActive, documentPages, wordPageCount]);
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (match) match.scrollIntoView({ behavior: "auto", block: 'center', inline: 'nearest' });
      else if (target.page) goToPage(target.page);
    });
  }, [pdf, searchIndex, searchMatches, viewerSearch, viewerSearchActive, wordPageCount]);
  ...
  const goToPage = (next, { smooth = true } = {}) => {
    ...
    requestAnimationFrame(() => {
      ...
      const rootRect = root.getBoundingClientRect();
      const nodeRect = node.getBoundingClientRect();
      const targetTop = Math.max(0, nodeRect.top - rootRect.top + root.scrollTop - 4);
  ...
  const shareDocument = async () => {
    try {
      if (navigator.share) await navigator.share({ title: doc?.name, url: window.location.href });
  ...
  const currentSearchText = searchMatches.length ? `${searchIndex + 1}/${searchMatches.length}` : viewerSearch.trim().length >= 2 ? '0/0' : '—';
    ...
    return (
      <section className="surface flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-[#111a2d]">
        <div className="max-w-[430px] px-5 text-center">
          <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-3xl bg-blue-50 text-blue-500 dark:bg-blue-950/50"><FileText size={38} /></div>
          <h2 className="text-[21px] font-bold">No document selected</h2>
          <p className="mt-2 text-[13px] leading-6 text-slate-500">Select a document from the left panel to view its content or search across your documents to find relevant information.</p>
          <div className="mx-auto mt-6 rounded-lg border border-blue-200 bg-blue-50/50 p-4 text-left dark:border-blue-900 dark:bg-blue-950/20">
            <div className="flex gap-3"><Sparkles size={20} className="shrink-0 text-blue-600" /><div><b className="text-sm">Tip</b><p className="mt-1 text-xs text-slate-500">Use the search bar above to quickly find information across your uploaded documents.</p></div></div>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section ref={viewerRef} className="surface relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-[#111a2d]">
      <div className="flex min-h-[62px] shrink-0 items-center border-b border-slate-100 px-4 dark:border-slate-800">
        <FileIcon type={doc.type} />
        <div className="ml-3 min-w-0"><div className="truncate text-[16px] font-bold">{doc.name}</div><div className="text-[11px] text-slate-500">{doc.size}<span className="mx-1.5">•</span>{pageCountText}<span className="mx-1.5">•</span>{doc.date}</div></div>
        <div className="ml-auto flex items-center gap-2">
          <button className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-violet-500 to-blue-500 px-4 py-2 text-[12px] font-semibold text-white"><Sparkles size={15} />Ask AI</button>
          <button onClick={downloadDocument} className="flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-[12px] dark:border-slate-700"><Download size={15} />Download</button>
          <button onClick={shareDocument} className="flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-[12px] dark:border-slate-700"><Link size={15} />Share</button>
          <div className="relative"><button ref={viewerMenuButtonRef} onClick={() => setMenu(v => !v)} className="rounded-lg border border-slate-200 p-2 dark:border-slate-700" aria-label="Document actions"><MoreVertical size={17} /></button>{menu && <div ref={viewerMenuRef} className="absolute right-0 top-11 z-50 w-40 rounded-lg border border-slate-200 bg-white p-1 shadow-xl dark:border-slate-700 dark:bg-slate-900"><Menu label="Open in new tab" onClick={openInNewTab} /><Menu label="Rename" /><Menu label="Move to folder" /><Menu label="Close document" red onClick={() => { setMenu(false); onClose(); }} /><Menu label="Delete" red /></div>}</div>
        </div>
      </div>

      <div className="flex h-10 shrink-0 items-end gap-7 border-b border-slate-100 px-4 dark:border-slate-800">
        {['Viewer', 'Summary', 'Key Insights', 'Related Content'].map(t => <button key={t} onClick={() => setTab(t)} className={`h-10 whitespace-nowrap text-[12px] ${tab === t ? 'border-b-2 border-blue-600 font-semibold text-blue-600' : 'text-slate-500'}`}>{t}</button>)}
      </div>

      {tab !== 'Viewer' ? <div className="min-h-0 flex-1 overflow-auto p-8"><h2 className="text-xl font-bold">{tab}</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-500">Mock {tab.toLowerCase()} generated from the selected document. This area is ready to be connected to your RAG backend.</p></div> : (
        <>
          <div className="flex min-h-[44px] shrink-0 items-center border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center border-r border-slate-100 px-3 dark:border-slate-800">
              <button onClick={() => goToPage(page - 1)} disabled={page <= 1} className="disabled:opacity-30"><ChevronLeft size={17} /></button>
              <button onClick={() => { const value = window.prompt('Go to page', String(page)); if (value) goToPage(value); }} className="mx-2 rounded bg-slate-100 px-2 py-1 text-[12px] dark:bg-slate-800">{page}</button>
              <span className="text-[12px]">/ {actualPageCount}</span>
              <button onClick={() => goToPage(page + 1)} disabled={page >= actualPageCount} className="ml-2 disabled:opacity-30"><ChevronRight size={17} /></button>
            </div>
            <div className="flex items-center gap-2 border-r border-slate-100 px-3 dark:border-slate-800">
              <button onClick={() => changeZoom(-10)} disabled={zoom <= MIN_ZOOM} className="disabled:opacity-30" title="Zoom out"><Minus size={16} /></button>
              <span className="rounded bg-slate-100 px-3 py-1 text-[12px] dark:bg-slate-800">{zoom}%</span>
              <button onClick={() => changeZoom(10)} disabled={zoom >= MAX_ZOOM} className="disabled:opacity-30" title="Zoom in"><Plus size={16} /></button>
              <button onClick={resetZoom} disabled={zoom === 100} className="rounded px-1.5 text-[11px] font-medium text-slate-500 hover:text-slate-800 disabled:opacity-30 dark:hover:text-slate-200" title="Reset zoom">Reset</button>
            </div>
            ...
            <button onClick={toggleFullscreen} className="mx-3" title={fullscreen ? 'Exit fullscreen' : 'Fullscreen'}>{fullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}</button>
            <div className="ml-auto mr-3 flex h-8 items-center justify-end gap-2 text-[11px] text-slate-500">
              <div className={`flex h-8 items-center overflow-hidden transition-[width,opacity] duration-200 ease-in-out ${searchOpen ? 'w-[250px] max-w-[35vw] opacity-100' : 'w-0 opacity-0 pointer-events-none'}`}>
                <div className="flex h-8 w-[250px] max-w-[35vw] items-center gap-2 rounded border border-slate-200 px-2 dark:border-slate-700">
                  ...
                  <input
                    ...
                    value={viewerSearch}
                    onChange={e => { setViewerSearch(e.target.value); setViewerSearchActive(true); }}
            ...
            {searchMatches.length > 1 && <>
              <button onClick={() => setSearchIndex(i => (i - 1 + searchMatches.length) % searchMatches.length)} className="mr-1 text-slate-500" title="Previous match"><ChevronLeft size={15} /></button>
              <button onClick={() => setSearchIndex(i => (i + 1) % searchMatches.length)} className="mr-2 text-slate-500" title="Next match"><ChevronRight size={15} /></button>
            </>}
            <button onClick={() => setRefreshKey(v => v + 1)} className="mr-3 text-slate-500" title="Refresh page"><RefreshCcw size={15} /></button>
          </div>

          <div className="flex min-h-0 flex-1 overflow-hidden">
            <aside ref={thumbnailRailRef} className="thin-scroll w-[112px] shrink-0 overflow-y-auto bg-slate-50 p-2 dark:bg-slate-900">
              <div ref={thumbnailContentRef} className="relative">
                ...
                />
              {pdf && Array.from({ length: actualPageCount }, (_, i) => i + 1).map(p => (
                <button
                  key={p}
                  ref={node => { if (node) thumbnailRefs.current.set(p, node); else thumbnailRefs.current.delete(p); }}
                  onClick={() => goToPage(p, { smooth: true })}
                  className="mb-3 block w-full"
                >
                  <div data-thumbnail-box className="mx-auto flex h-[113px] w-[72px] items-center justify-center overflow-hidden rounded border border-slate-200 bg-white">
                    <PdfPage pdf={pdf} pageNumber={p} thumbnail render={true} />
                  </div>
                  <div className={`mt-1 text-center text-[10px] ${page === p ? 'text-blue-600' : ''}`}>{p}</div>
                </button>
              ))}
              {doc.type === 'txt' && Array.from({ length: actualPageCount }, (_, i) => i + 1).map(p => (
                <button
                  key={p}
                  ref={node => { if (node) thumbnailRefs.current.set(p, node); else thumbnailRefs.current.delete(p); }}
                  onClick={() => goToPage(p, { smooth: true })}
                  className="mb-3 block w-full"
                >
                  <div data-thumbnail-box className="mx-auto h-[113px] w-[72px] overflow-hidden rounded border border-slate-200 bg-white p-1 text-left text-[8px] leading-3 text-slate-500">
                    {String(documentPages[p - 1] || '').slice(0, 360)}
                  </div>
                  <div className={`mt-1 text-center text-[10px] ${page === p ? 'text-blue-600' : ''}`}>{p}</div>
                </button>
              ))}
              {isWordDocument(doc) && Array.from({ length: actualPageCount }, (_, i) => i + 1).map(p => (
                <button
                  key={p}
                  ref={node => { if (node) thumbnailRefs.current.set(p, node); else thumbnailRefs.current.delete(p); }}
                  onClick={() => goToPage(p, { smooth: true })}
                  className="mb-3 block w-full"
                >
                  <div data-thumbnail-box className="mx-auto flex h-[113px] w-[72px] items-center justify-center overflow-hidden rounded border border-slate-200 bg-white text-[9px] font-medium text-slate-500">
                    <div className="px-1 text-center leading-3">A4<br/>Page {p}</div>
                  </div>
                  <div className={`mt-1 text-center text-[10px] ${page === p ? 'text-blue-600' : ''}`}>{p}</div>
                </button>
              ))}
            ...
            <div ref={pdfScrollRef} className="thin-scroll min-w-0 flex-1 overflow-auto bg-slate-100 p-4 sm:p-6 dark:bg-slate-950">
              <div className="mx-auto flex min-w-0 flex-col items-center gap-5 py-1">
                {doc.type === 'pdf' && pdf && (viewMode === 'page' ? [page] : pageElements).map(p => {
                  const size = pageSizes[p - 1];
                  const width = size ? size.width * (zoom / 100) : 720 * (zoom / 100);
                  const height = size ? size.height * (zoom / 100) : 930 * (zoom / 100);
                  const shouldRender = viewMode === 'page' || viewerSearchActive || renderedPages.has(p);
                  return (
                    <div
                      key={p}
                      ref={node => { if (node) pageRefs.current.set(p, node); else pageRefs.current.delete(p); }}
                      data-page={p}
                    ...
                    >
                      {shouldRender ? (
                        ...
                        searchMatchOffset={viewerSearchActive ? searchMatches.filter(m => m.page < p).length : 0}
                        currentSearchMatch={viewerSearchActive ? searchIndex : -1}
                      />
                      ) : (
                        <div className="absolute inset-0 flex items-center justify-center text-[11px] text-slate-300">Page {p}</div>
                      )}
                    </div>
                  );
                })}
                {doc.type === 'pdf' && !pdf && !pdfError && <div className="flex h-[520px] w-[720px] max-w-full items-center justify-center rounded bg-white text-sm text-slate-500 shadow-sm">Loading PDF…</div>}
                {doc.type === 'pdf' && pdfError && <div className="flex h-[520px] w-[720px] max-w-full flex-col items-center justify-center rounded bg-white text-sm text-red-500 shadow-sm"><div>Unable to load the PDF.</div><button onClick={() => setRefreshKey(v => v + 1)} className="mt-3 rounded border px-3 py-1 text-xs text-slate-600">Retry</button></div>}
                {doc.type === 'txt' && (viewMode === 'page' ? [page] : documentPages.map((_, i) => i + 1)).map(p => {
                    ...
                    >
                      ...
                      >
                        <pre className="m-0 whitespace-pre-wrap break-words font-sans text-[15px] leading-7">
                          {highlightPlainText(content, viewerSearch, documentPages.slice(0, p - 1).reduce((total, pageText) => {
                            const needle = viewerSearch.toLocaleLowerCase();
                            if (!needle) return total;
                            const lower = (pageText || '').toLocaleLowerCase();
                            let count = 0;
                            let at = lower.indexOf(needle);
                            while (at !== -1) { count += 1; at = lower.indexOf(needle, at + Math.max(1, needle.length)); }
                            return total + count;
                          }, 0), searchIndex)}
                        </pre>
                      </div>
...
function Menu({ label, red, onClick }) {
  return <button onClick={onClick} className={`w-full rounded px-2 py-2 text-left text-xs hover:bg-slate-100 dark:hover:bg-slate-800 ${red ? 'text-red-500' : ''}`}>{label}</button>;
}
